...
def solution(n, lost, reserve):
    ...
    for num in lost:        # 체육복을 도난 당했지만 여벌 체육복을 빌릴 수 있으면 lost에서 제거 
        ...
        if reserve.count(num) == 1:     # 내가 여벌을 갖고 있는 경우 
            max += 1
            reserve.remove(num)
        elif reserve.count(num-1) == 1:     # 내 앞번호가 여벌을 갖고 있는 경우
            max += 1
            reserve.remove(num-1)
        elif reserve.count(num+1) == 1:     # 내 뒷번호가 여벌을 갖고 있는 경우
            max += 1
            reserve.remove(num+1)

    answer = n - len(lost) + max       # 최대 인원 구하기 
    
    return answer   # 체육수업을 들을 수 있는 학생의 최댓값 


answer = solution(5,[2,4],[1,3,5])
#answer = solution(5,[2,4],[3])
#answer = solution(3,[3],[1])
print(answer)